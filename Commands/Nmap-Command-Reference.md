...
Firewalls and network filtering devices can affect how Nmap interprets the state of a port.

Instead of receiving a direct response from the target service, Nmap may receive:

- A TCP response from the target
- An ICMP error message
- No response at all
- A response modified or blocked by a firewall

Understanding these responses is important when interpreting Nmap scan results.
...
| **Closed** | The host is reachable, but no application is listening on the port. |
| **Filtered** | A firewall or filtering device prevents Nmap from determining whether the port is open. |
| **Open\|Filtered** | Nmap cannot determine whether the port is open or filtered because no response clearly establishes the state. |
| **Closed\|Filtered** | Nmap cannot determine whether the port is closed or filtered based on the responses received. |

### How Firewalls Affect Scanning

A firewall can influence scan results by:

- Dropping packets silently
- Rejecting packets with TCP reset or ICMP responses
- Allowing some protocols while blocking others
- Filtering traffic based on source or destination
- Rate-limiting responses
- Applying stateful inspection rules

For example, a TCP SYN scan may normally receive a SYN/ACK from an open port. If a firewall silently drops the probe, Nmap may classify the port as **filtered** rather than **open** or **closed**.

### Example

```bash
nmap -sS -p 22,80,443 192.168.1.15
```
...
```
- Port 22 responded in a way consistent with an open service.
- Port 80 did not provide enough information to determine whether it is open because filtering may be present.
- Port 443 responded in a way consistent with a closed port.

### Using ACK Scans to Analyze Filtering

The TCP ACK scan (-sA) is particularly useful for studying firewall filtering behavior.

```
nmap -sA -p 22,80,443 192.168.1.15
```

An ACK scan does not determine whether a port is open. Instead, it helps determine whether ports are **filtered or unfiltered.**
...
This can provide additional information about how packet-filtering rules affect the target.
...
- Understanding filtered port states
- Validating network segmentation
- Investigating unexpected scan results

### Security Insight

A filtered port does not necessarily mean that no service exists behind the firewall.

It means that Nmap cannot obtain enough information to determine the port's state because filtering prevents a definitive response.

Security professionals should therefore avoid treating filtered as equivalent to closed.

### Detection & Defensive Considerations

Firewalls can reduce reconnaissance visibility by filtering unsolicited traffic, but filtering alone does not prevent scanning activity from being detected.
...
to identify and respond to reconnaissance activity.

Understanding Nmap's port-state classifications also helps defenders interpret scan activity and troubleshoot firewall policies.
...
## Output to All Major Formats (`-oA`)

### Purpose

Saves Nmap scan results in multiple output formats at the same time.
...
### Example
```
nmap -oA scan-results 192.168.1.15
```

### How It Works

The **-oA** option saves the scan using the specified base filename in three major formats:
...
- Automated analysis
- Technical reporting
- Long-term storage of scan results

### Advantages

- Creates multiple useful formats with one command.
- Provides both human-readable and machine-readable results.
- Useful for professional assessment workflows.

### Limitations
- Produces multiple files for each scan.
- Grepable output is a legacy format and is less suitable for advanced automation than XML.

### Security Insight

Using **-oA** is useful when scan results need to support both manual analysis and automated processing. It is particularly valuable when preserving evidence for later reporting.

## Detection & Defensive Considerations

The output format does not affect the network traffic generated by the scan. Defensive systems detect the scanning activity rather than the format used to save its results.
...
Provides additional information about the scan while Nmap is running.
...
```
nmap -v 192.168.1.15
nmap -vv 192.168.1.15
```

### How It Works

The **-v** option increases the amount of information Nmap displays during the scan.

Using **-vv** increases the verbosity further and provides additional scan progress and diagnostic information.

## When to Use
- Monitoring scan progress
- Troubleshooting scans
- Understanding Nmap's behavior
- Laboratory exercises
- Long-running assessments

### Advantages
- Provides greater visibility into scan progress.
- Helps identify what Nmap is doing during a scan.
- Useful for troubleshooting.

### Limitations
- Produces more output.
- Excessive verbosity can make results harder to read.
- Does not improve the actual scanning capability.

### Security Insight

Verbose output is useful during security assessments because it helps the tester understand scan progress and identify unexpected behavior without changing the underlying scan objective.

### Detection & Defensive Considerations

Verbosity only affects what Nmap displays to the operator. It does not change the network traffic generated by the scan.
...
Displays the reason Nmap assigned to each reported host or port state.
...
### Example
```
nmap --reason 192.168.1.15
```

### How It Works

The **--reason** option tells Nmap to explain why it assigned a particular state to a host or port.

For example, Nmap may indicate that a port is open because it received a syn-ack response or that a port is closed because it received a reset response.

### When to Use
- Analyzing scan results
- Troubleshooting unexpected port states
- Studying firewall behavior
- Security assessments
- Learning how Nmap interprets network responses

### Advantages
- Makes scan results easier to interpret.
- Helps connect port states to observed network responses.
- Useful for troubleshooting and learning.

### Limitations
- Adds additional information to the output.
- Does not change the scanning technique itself.
- The reported reason is based on the responses available to Nmap.

### Security Insight

Understanding why Nmap reports a port as open, closed, or filtered helps security professionals distinguish between actual service states and network filtering behavior.

### Detection & Defensive Considerations

The **--reason** option only changes the information displayed in the scan results. It does not alter the probes sent to the target.
...
Appends new Nmap scan results to an existing output file instead of replacing its previous contents.

### Syntax

```bash
nmap --append-output -oN <output-file> <target>
```
---

### Example
```
nmap --append-output -oN scan-history.txt 192.168.1.15
```

### How It Works

Normally, saving Nmap output to an existing filename can overwrite the previous contents.

The **--append-output** option tells Nmap to add the new scan results to the existing file.

This allows multiple scan results to be stored in the same output file.

### When to Use
-- Repeated assessments
- Monitoring changes over time
- Maintaining scan history
- Comparing results from multiple assessments
- Laboratory experiments

### Advantages
- Preserves previous scan results.
- Useful for maintaining historical records.
- Supports repeated scanning workflows.

### Limitations
- The file can become difficult to manage after many scans.
- Results from different assessments may become mixed together.
- Separate files may be preferable when strict evidence separation is required.

### Security Insight

Maintaining historical scan results can help identify changes in exposed services, network configuration, and attack surface over time.

### Detection & Defensive Considerations

Appending output only changes how results are stored locally. It does not affect the traffic generated by the scan.

---

## Scan Statistics (`--stats-every`)

### Purpose

Displays periodic progress statistics while an Nmap scan is running.
...
### Example
```
nmap --stats-every 30s 192.168.1.15
```

### How It Works

The **--stats-every** option instructs Nmap to periodically display scan progress information at the specified interval.

The information can help the operator understand how much of the scan has been completed and how much time remains.
...
- Useful for lengthy scans.
- Helps estimate remaining scan time.

### Limitations
- Produces additional console output.
- Does not make the scan faster.
- Mainly useful for monitoring rather than scan optimization.

### Security Insight

Progress statistics are particularly useful during large authorized assessments because they help the tester monitor the assessment without interrupting the scan.

### Detection & Defensive Considerations

This option affects local scan-status output only and does not change the network traffic generated by Nmap.

---


## Resume an Interrupted Scan (`--resume`)

### Purpose

Resumes an interrupted Nmap scan from a previously saved normal-output file.
...
If an Nmap scan is interrupted, the --resume option can continue the scan using information stored in the previously generated normal-output file.

This is particularly useful for long-running scans that were interrupted before completion.

### When to Use

- Long-running network scans
- Interrupted assessments
- Large subnet scans
- Laboratory exercises
- Recovering from an interrupted scan

### Advantages

- Avoids restarting certain interrupted scans from the beginning.
- Saves time during lengthy assessments.
- Useful when scanning large numbers of hosts or ports.

### Limitations

- Requires an appropriate saved Nmap output file.
- Cannot resume every possible scan state.
- The original scan must have produced the information required for resumption.

### Security Insight

The ability to resume scans is useful during professional assessments because large network scans can take significant time. Preserving scan output allows work to continue after an interruption.

- Detection & Defensive Considerations

Resuming a scan generates network traffic as the scan continues. Defensive monitoring can still detect the resumed scanning activity.
...
Sets the initial round-trip time (RTT) timeout that Nmap uses when waiting for probe responses.
...
### Example
```
nmap --initial-rtt-timeout 500ms 192.168.1.15
```

### How It Works

Nmap uses RTT measurements to determine how long it should wait for responses from a target.

The initial RTT timeout provides the starting value before Nmap adjusts its timing based on observed network conditions.

### When to Use
- Controlled laboratory testing
- High-latency network assessments
- Performance experimentation
- Troubleshooting scan timing

### Advantages
- Provides greater control over probe timing.
- Can help tune scans for known network conditions.

### Limitations
- Incorrect values may cause unnecessary delays.
- Values that are too aggressive may contribute to missed responses.
- Nmap normally adjusts timing automatically.

### Security Insight

RTT behavior is an important factor in reliable network scanning. Understanding how Nmap determines response timeouts helps security professionals distinguish between genuine filtering and delayed network responses.

### Detection & Defensive Considerations

Changing the initial RTT timeout does not inherently make a scan less detectable. The resulting packet activity remains visible to network monitoring systems.
...
Sets the maximum amount of time Nmap will wait for a probe response before considering the probe to have timed out.
...
### Example
```
nmap --max-rtt-timeout 2s 192.168.1.15
```

### How It Works

Nmap dynamically adjusts its response timeout based on network conditions.

The **--max-rtt-timeout** option places an upper limit on that timeout.

### When to Use
- High-latency network testing
- Controlled performance tuning
- Large-scale authorized assessments
- Troubleshooting slow responses

### Advantages
- Provides control over maximum probe waiting time.
- Can prevent excessively long waits.

### Limitations
- Values that are too low may cause incomplete results.
- Values that are too high can increase scan duration.
- Manual tuning may interfere with Nmap's normal adaptive timing.

### Security Insight

Maximum RTT timeout settings should be selected according to the expected network latency. Reliable results require enough time for legitimate responses to arrive.

### Detection & Defensive Considerations

The timeout value primarily affects scanner behavior and scan duration. It does not directly hide the generated traffic from defensive monitoring systems.
...
Attempts to maintain a minimum average packet transmission rate during a scan.
...
### Example
```
nmap --min-rate 100 192.168.1.15
```

How It Works

The ***--min-rate** option tells Nmap to attempt to send packets at no slower than the specified average rate.

This can be useful when the scanner needs to maintain a predictable minimum scanning speed.

### When to Use
- Controlled laboratory environments
- High-speed internal networks
- Large authorized assessments
- Performance benchmarking

### Advantages
- Can reduce scan duration.
- Provides greater control over scanning speed.
- Useful for controlled performance testing.

### Limitations
- Excessive rates can cause packet loss.
- May overwhelm network devices or hosts.
- Can increase detection likelihood.
- Should be used carefully on production networks.

### Security Insight

Scan speed should always be balanced against reliability and network impact. Increasing the packet rate does not necessarily produce better results if the target or network begins dropping probes.

### Detection & Defensive Considerations

Higher packet rates create more concentrated traffic and can make reconnaissance activity easier for IDS, IPS, firewalls, and network monitoring systems to identify.
...
Limits the maximum average packet transmission rate during a scan.
...
### Example
```
nmap --max-rate 50 192.168.1.15
```

### How It Works

The **--max-rate** option limits how quickly Nmap sends packets.

This allows the tester to control the amount of traffic generated during an authorized assessment.

### When to Use
- Bandwidth-constrained networks
- Production environments where scan impact must be controlled
- Laboratory experiments
- Rate-sensitive network devices

### Advantages
- Helps control network load.
- Reduces the risk of generating excessive traffic.
- Provides predictable scan-rate limits.

### Limitations
- Lower rates increase scan duration.
- Extremely low values may make scans inefficient.
- Does not guarantee that packets will never be dropped.

### Security Insight

Controlling scan rate is an important part of responsible security testing. A slower scan may be preferable when network availability and stability are more important than completing the assessment quickly.

### Detection & Defensive Considerations

Reducing packet rate can reduce burst traffic, but it does not make scanning invisible. Defensive systems can correlate reconnaissance activity over time.

---

## Host Group Size (`--min-hostgroup` / `--max-hostgroup`)

### Purpose

Controls the minimum and maximum number of hosts that Nmap attempts to scan together when scanning multiple targets.
...
When scanning multiple hosts, Nmap groups targets together and scans them in batches.

These options allow the tester to influence the size of those host groups.

Larger groups can improve efficiency on reliable networks, while smaller groups can provide more controlled scanning behavior.
...
### Advantages
- Can improve efficiency when scanning many hosts.
- Provides greater control over large-scale scans.
- Useful for performance testing.

### Limitations
- Larger groups can increase simultaneous network activity.
- Smaller groups may increase total scan duration.
- Nmap normally manages host grouping automatically.

### Security Insight

Host grouping is particularly relevant when scanning an entire subnet or enterprise environment. The objective should be to balance scan completion time with network stability and assessment requirements.

### Detection & Defensive Considerations

Scanning many hosts simultaneously can create a noticeable pattern of reconnaissance traffic. Network monitoring systems can identify coordinated probes across multiple hosts.

---

## Defeat TCP RST Rate Limiting (`--defeat-rst-ratelimit`)

### Purpose

Helps TCP scans deal with targets that limit the rate at which they send TCP reset (RST) responses.
...
### Example
```
nmap --defeat-rst-ratelimit -sS 192.168.1.15
```

### How It Works

Some operating systems and network devices limit the number of TCP reset responses they send within a period of time.

This can make closed ports appear as filtered when scanning large numbers of ports.

The option tells Nmap to work around the effects of TCP RST rate limiting by reducing reliance on those responses when determining scan behavior.

### When to Use
- TCP scanning of rate-limited systems
- Large port scans
- Network performance testing
- Authorized laboratory assessments

### Advantages
- Can improve scan performance against systems with RST rate limiting.
- Helps reduce misleading results caused by response-rate controls.

### Limitations
- May cause some ports to be reported as open|filtered.
- Can reduce certainty in port-state classification.
- Should not be used without understanding the target's response behavior.

### Security Insight

Rate limiting is an important defensive mechanism because it can reduce the amount of information available to scanners. Understanding its effect helps security professionals correctly interpret unusual Nmap results.

### Detection & Defensive Considerations

TCP reset rate limiting can make reconnaissance more difficult but does not prevent scanning. Network monitoring can still identify the probes generated by the scanner.

---

## Defeat ICMP Rate Limiting (`--defeat-icmp-ratelimit`)

### Purpose

Helps UDP scans deal with targets that limit the rate of ICMP port-unreachable responses.

### Syntax

```bash
nmap --defeat-icmp-ratelimit -sU <target>
```
---

### Example
```
nmap --defeat-icmp-ratelimit -sU 192.168.1.15
```
### How It Works

UDP scans often rely on ICMP port-unreachable responses to identify closed UDP ports.

Some systems limit the rate of these ICMP responses. When this occurs, Nmap may have difficulty determining the state of UDP ports accurately.

This option changes Nmap's handling of the effects of ICMP rate limiting during UDP scans.

### When to Use
- UDP scanning
- Systems with ICMP response rate limiting
- Network troubleshooting
- Authorized laboratory assessments

### Advantages
- Can improve UDP scan efficiency against rate-limited systems.
- Helps address incomplete information caused by ICMP response limits.

### Limitations
- May reduce confidence in UDP port-state results.
- Does not eliminate the effects of filtering.
- UDP scans can still be significantly slower than TCP scans.

### Security Insight

ICMP rate limiting demonstrates how defensive controls can affect reconnaissance results without completely blocking network traffic. Security professionals should consider response-rate behavior when interpreting UDP scan results.

### Detection & Defensive Considerations

ICMP rate limiting can reduce reconnaissance information available to scanners, but UDP probe traffic may still be visible to firewalls and network monitoring systems.
